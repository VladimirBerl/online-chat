...
export default class Block<TProps extends TBlockProps = {}> {
  ...
  _props: TProps;
  _childs: Record<string, Block | Block[]>;
  _id: string;
  ...
  _render() {
    const template = this.render().firstElementChild as HTMLElement;
  ...
  getChildsAndProps(childsAndProps: TProps) {
    const childs: Record<string, Block | Block[]> = {};
  ...
  compile(template: (context: unknown, options?: unknown) => string, props?: TProps): DocumentFragment {
    ...
    Object.entries(this._childs).forEach(([key, child]) => {
      if (Array.isArray(child)) {
        // @ts-ignore
        propsAndStubs[key] = child.map((item: Block) => `<div data-id="${item._id}"></div>`);
      } else {
        // @ts-ignore
