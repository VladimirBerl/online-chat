...
export default class Validator {
  ...
  handleInput = (event: Event) => {
    const field = this.fieldList.find((item) => item.getElement().querySelector("input") === event.target);
  ...
  validate(field: Block) {
    ...
    if (Array.isArray(rules)) {
      ...
      const errors = Object.entries(methods).reduce((acc, [key, method]) => {
        return typeof method === "function" && !method(input?.value) ? [...acc, key] : acc;
      }, []);
  ...
  toggleErrors(field: Block, inputName: string) {
    ...
    if (typeof errors !== "undefined") {
      field.setProps({
        errors: errors.length !== 0 ? pickProps(this.messages, errors) : null,
      });
    }
  }
